...
public class ResourceCategoryVocabService extends SISSVoc3Service {
	...
	public Map<String, String> getAllResourceCategoryConcepts() throws Exception {
		...
		do {
			HttpRequestBase method = sissVocMethodMaker.getAllConceptsInScheme(getBaseUrl(), getRepository(),
					VocabularyLookup.RESOURCE_CATEGORY.scheme(), Format.Rdf, View.basic, pageSize, pageNumber);
	...
	public Map<String, String> getJorcResourceCategoryConcepts() throws Exception {
		...
		do {
			HttpRequestBase method = sissVocMethodMaker.getAllConceptsInScheme(getBaseUrl(), getRepository(),
					VocabularyLookup.RESOURCE_CATEGORY.scheme(), Format.Rdf, View.description, pageSize, pageNumber);
			if (requestPageOfConcepts(method, model)) {
				pageNumber++;
			} else {
				break;
