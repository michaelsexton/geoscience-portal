    ...
    _generateQuerier : function(knownLayer, wfsResources, wmsResources, wcsResources, irisResources) {
        ...
        if(wfsResources.length > 0 && wmsResources.length > 0){
            ...
            for (var i = 0; i < wmsResources.length; i++) {
                if (wmsResources[i].get('name') === 'Onshore_Seismic_Surveys') {
                    return Ext.create('portal.layer.querier.wms.OnshoreSeismicSurveyQuerier', cfg);
                }
            }
            ...
            if (knownLayer && knownLayer.get('id') === '250K-scanned-geological-maps') {
                return Ext.create('portal.layer.querier.wms.ScannedGeologicalMapsQuerier', cfg);
            }
